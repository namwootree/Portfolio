...
## 느낀 점
 ...
 ### 1. 데이터 셋 이해의 어려움
  ...
  - Scrabble Game에 대한 이해가 부족하여 데이터의 Features가 가진 의미를 이해하는 데 어려움이 존재
  - Scrabble Game 규칙에 관한 설명 영상과 글을 보며 데이터를 이해
  - 이를 통해 각각 Features가가 어떻게 Score에 영향을 미치는지 이해할 수 있어으며 이를 통해 다양한 파생변수 생성할 수 있게 됨

### 2. 전처리

  - Reference를 참고하여 전처리를 시도하였다
  - 게임 규칙에 대한 이해를 바탕으로 특별 보너스 점수(X2 or X3)에 대한 파생변수를 생성하였지만 성능 향상에는 도움이 되지 못하였다

### 3. 모델링

  - LightGBM외에 XGBoost와 Catboost를 활용하였지만 성능 향상시키지 못하였다
  - Stacking Ensemble을 시도하였지만 성능향상시키지 못하였다
  - Repeated KFold외에 Group KFold를 시도하였지만 성능 향상시키지 못하였다
      * 그러나 Group KFold의 원리와 사용한 이유를 공부하는 과정에서 다른 Project에서는 도움이 될 것으로 기대함

### 4. Reference의 양날의 검

  - 이번 프로젝트를 통해 가장 중요하게 느낀 점이다

  #### 장점
  
   - Reference를 통해 다양한 시각화와 전처리 시도를 배울 수 있었다
   - 이를 통해 이번 프로젝트의 Process를 쉽게 구축할 수 있었다
   - Reference에 대한 조금의 변형으로 성능을 높일 수 있었다

  #### 단점
  
   - 처음부터 시작하여 여러번 좌절을 겪는 과정에서 오는 깨달음 (자신만의 노하우 및 방법론 등)을 얻을 수 없었다
   - Reference가 처음부터 좋은 성적을 낸 결과, 사고의 틀이나 프로젝트 방향성이 Reference 틀에서 벗어나지 못하였다
      * EX : "'하이퍼파리미터만 바꾸면' 혹은 '변수만 조금 더 추가하면' 성능이 향상될꺼야"라는 크게 성장시키지 못한 사고 (공부하고자 프로젝트 한 것이데 본질을 잃음)
